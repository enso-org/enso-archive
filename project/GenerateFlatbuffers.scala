...
object GenerateFlatbuffers {
  ...
  /**
    * Runs flatc to ensure that it is properly installed and checks if the reported version is consistent with
    * expectations.
    *
    * @param expectedVersion flatc version that is expected to be installed, should be based on project settings
    */
  ...
  /**
    ...
    * flatc will fail if there are errors in the files, as this is the first time it is run on the files, the errors are
    * captured and reported (see Note [flatc Error Reporting]).
    *
    ...
    * @return set of files that will be generated by running flatc on provided schemas
    */
