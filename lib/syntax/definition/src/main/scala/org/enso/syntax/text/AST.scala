...
object Shape extends ShapeImplicit {
  ...
  final case class ListLiteral[T](items: List[T]) extends SpacelessAST[T]
  ...
  object ListLiteral {
    implicit def ftor: Functor[ListLiteral]  = semi.functor
    implicit def fold: Foldable[ListLiteral] = semi.foldable
    implicit def repr[T: Repr]: Repr[ListLiteral[T]] =
      t => R + "[" + t.items.map(_.repr.build()).mkString(", ") + "]"
    implicit def ozip[T]: OffsetZip[ListLiteral, T] = _.map(Index.Start -> _)
    implicit def span[T]: HasSpan[ListLiteral[T]]   = _ => 0
...
sealed trait ShapeImplicit {
  ...
  implicit def ozip[T: HasSpan]: OffsetZip[Shape, T] = {
    ...
    case s: ListLiteral[T] => OffsetZip[ListLiteral, T].zipWithOffset(s)
...
object AST {
  ...
  type ListLiteral = ASTOf[Shape.ListLiteral]
  object ListLiteral {
    val any = UnapplyByType[ListLiteral]
