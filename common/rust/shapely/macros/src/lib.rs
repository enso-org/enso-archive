//! This module defines a custom derive macro `Iterator`.
//!
//! The `#[derive(Iterator)]` can be used on any struct type that takes at least
//! a single type parameter. For `struct Foo` it will provide:
...
/// Macro entry point, decides on which type parameter will be used to detect
/// iterable fields.
